...
public class LevelFinishedController implements Initializable{
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		int level = Launch.getGame().getLevelNumber();
		
		username.setCellValueFactory(new PropertyValueFactory<>("Username"));
		time.setCellValueFactory(new PropertyValueFactory<>("Time"));
		score.setCellValueFactory(new PropertyValueFactory<>("Score"));
		
		PriorityQueue<UserProfile> queue = new PriorityQueue<UserProfile>(board.getLevelTimes(level));
		
		int i = 0;
		while(!queue.isEmpty() && i < 3) {
			UserProfile user = queue.remove();
			observableList.add(new Time(user.getLevelTime(level),user.getLevelHighScore(level),user.getUsername()));
			i++;
		}
